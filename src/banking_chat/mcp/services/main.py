...
async def create_service_request_tool(payload: CreateServicePayload) -> dict[str, Any]:
    return await handlers.create_service_request(payload.customer_id, {"type": payload.type, "notes": payload.notes})
...
async def block_card_tool(payload: BlockCardPayload) -> dict[str, Any]:
    return await handlers.block_card(
        payload.customer_id,
        {"card_last_four": payload.card_last_four, "reason": payload.reason, "block_type": payload.block_type},
    )
